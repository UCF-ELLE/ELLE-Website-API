...
class ModuleReport(Resource):
    ...
    def get(self):
        ...
        include_su_stats = getParameter('includeSuStats', bool, False, "Include any value with this key to include SU stats")
        include_pf_stats = getParameter('includePfStats', bool, False, "Include any value with this key to include PF stats")

        permission_options = "`user`.`permissionGroup` = 'st'"
        if permission == 'su' or permission == 'pf':
            if include_su_stats:
                permission_options = permission_options + " OR `user`.`permissionGroup` = 'su'"
            if include_pf_stats:
...
class TermsPerformance(Resource):
    ...
    def get(self):
        ...
        data['includeOwnStats'] = getParameter('includeOwnStats', bool, False, "Pass in any value with this parameter to include user's own stats (for superadmins and professors)")
