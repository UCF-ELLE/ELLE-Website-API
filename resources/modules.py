...
class SearchModules(Resource):
    ...
    def get(self):
        ...
        data = {}
        data["language"] = getParameter(
            "language", str, True, "please provide two letter languge code"
        )
...
class Module(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            moduleID = getFromDB(
                query
            )  # ADD A CHECK TO SEE IF IT RETURNED SUCCESSFULLY
    ...
    def delete(self):
        ...
        if (
            permission == "pf"
            and module_user_id not in TA_list
            and module_user_id != user_id
        ) or (permission == "ta" and module_user_id != user_id):
            return errorMessage("User not authorized to do this"), 401
...
class AttachQuestion(Resource):
    ...
    def post(self):
        ...
        # Parsing JSON
        module_id = getParameter(
            "moduleID", int, True, "moduleID in integer format is required"
        )
        ...
        try:
            ...
            if not result or not result[0]:
                ...
                if result and result[0]:
                    ...
                    postToDB(
                        """ INSERT INTO question (`type`, `questionText`) VALUES ("MATCH", "What is the translation of %s?")""",
                        front,
                    )
                    ...
                    postToDB(
                        "INSERT INTO answer (`questionID`, `termID`) VALUES (%s, %s)",
                        (question_id, term_id),
                    )
                else:
                    raise CustomException(
                        "Term does not exist or MATCH question has been deleted internally.",
                        400,
                    )
