...
export default function FixedAIForm({
    ...
    const [termCount, setTermCount] = useState<number>(5);
    ...
    const handleApproveTerms = async (approvedTerms: GeneratedTerm[]) => {
        ...
        try {
            ...
            for (const term of approvedTerms) {
                // Create term with moduleID to auto-attach
                const termData = {
            ...
            if (successCount === 0) {
                setStatusMessage('Module created but no terms were added. Check console for errors.');
                setSuccess(false);
                setCreatingModule(false);
                setStatus(true);
