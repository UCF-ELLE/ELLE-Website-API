...
export function ModuleStats() {
    ...
    const { response: moduleResponse, error: moduleError, loading: moduleLoading } = useAxios<ModuleStatsType>({
    ...
    const { response: languageResponse, error: languageError, loading: languageLoading } = useAxios<LanguageStatsType>({
        ...
        return (
            <Card style={{overflow: "scroll", height: "25vh", backgroundColor: "transparent", border: "none"}}>
                <Table className="statsTable"> 
                    ...
                    <tbody> 
                        {moduleData.modules.map((module, i) => {
                            return (
                                <tr key={i}>
    ...
    const getTermStats = (id: number) => {
        ...
        axios.get('elleapi/termsperformance', header)
        .then(res => {
            if (res.data.Message)
                setTermStats([])
            else 
